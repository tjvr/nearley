(function(root, factory) {
    if (typeof module === 'object' && module.exports) {
        module.exports = factory(require('bounded-set'));
    } else {
        root.nearley = factory();
    }
}(this, function(BoundedSet) {
...
function Tag(rule, dot) {
  this.id = ++Tag.highestId;

  // either a non-terminal...
  this.name = dot == rule.symbols.length ? rule.name : null;

  // ...or a dotted rule (i.e. LR0)
  this.expect = rule && rule.symbols[dot];
  this.next = null;
  this.rule = rule;
  this.dot = dot;
}
...
function BoundedMap(bound) {
  this._keys = new BoundedSet(bound);
  this._values = Object.create(null);
  this.has = this._keys.has.bind(this._keys);
}

BoundedMap.prototype.add = function(key, value) {
  this._keys.add(key|0);
  this._values[key|0] = value;
}

BoundedMap.prototype.addOnce = function(key, factory) {
  if (this._keys.has(key|0)) {
    throw 'moo';
    return this._values[key|0];
  } else {
    var value = factory(key|0);
...
Column.prototype.add = function(tag, reference, wantedBy) {
    ...
    var byTag = (this.byReference[reference] = this.byReference[reference] || new BoundedMap(Tag.highestId));
    var states = this.states;
    return byTag.addOnce(tag.id, function() {
      var s = new State(tag, reference, wantedBy);
      states.push(s);
      return s; // add to map
    })
}
    ...
    while (token = lexer.next()) {
        ...
        for (var w = scannable.length; w--; ) {
            ...
            if (expect.test ? expect.test(tokenOrCh) :
                ...
                // Add it
                var next = nextColumn.add(state.tag.next, state.reference, state.wantedBy);
